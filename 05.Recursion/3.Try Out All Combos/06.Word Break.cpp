/**
 * QUESTION: Given a string s and a dictionary of strings wordDict, return true if s can be 
 ...
 * 1. Start from the beginning of the string and try to find a word from the dictionary that
     matches a substring starting at the current index.
 ...
 * Time Complexity: O(2^N), where N is the length of the string. In the worst case, we can have 2^N recursive calls.
    At each index, we either extend the current word or treat the current substring as a valid word 
    and cut there. This leads to exploring all possible partitions of the string.
...
bool solve(int idx, string &s, unordered_set<string> &dict) {
    ...
    for (int i = idx; i < s.size(); i++) {
        temp.push_back(s[i]);
        if (dict.count(temp)) {
            if (solve(i + 1, s, dict)) return true;
        }
    }
    return false;
}
